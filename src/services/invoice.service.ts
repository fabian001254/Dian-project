...
export class InvoiceService {
  ...
  public async createInvoice(invoiceData: Partial<Invoice>, items: Partial<InvoiceItem>[]): Promise<Invoice> {
    ...
    try {
      ...
      // Map draft to DRAFT, final to PENDING (simulate pending approval)
      const statusToUse = invoiceData.status === 'draft'
        ? InvoiceStatus.DRAFT
      ...
      Object.assign(invoice, {
        ...
        dueDate: invoiceData.dueDate || this.calculateDueDate(new Date(), 30)
      });
      ...
      // Schedule automatic approval after random delay (5-30s)
      if (statusToUse === InvoiceStatus.PENDING) {
        const delayMs = Math.floor(Math.random() * (30000 - 5000 + 1)) + 5000;
        setTimeout(async () => {
          try {
            const repo = AppDataSource.getRepository(Invoice);
            savedInvoice.status = InvoiceStatus.APPROVED;
            await repo.save(savedInvoice);
            console.log(`Factura ${savedInvoice.id} auto-aprobada tras ${delayMs}ms`);
          } catch (err) {
            console.error('Error en auto-aprobación:', err);
          }
        }, delayMs);
      }
  ...
  public async sendInvoiceToDian(invoiceId: string): Promise<Invoice> {
    try {
      ...
      if (dianResponse.success) {
        ...
        // Assign random CUFE
        invoice.cufe = crypto.randomBytes(16).toString('hex').toUpperCase();
        // Set payment date
  ...
  public async getCompanyInvoices(
    ...
    if (customerId) {
      query = query.andWhere('invoice.customerId = :customerId', { customerId });
    }
   ...
   */
  public async generateAndSaveXML(invoice: Invoice): Promise<string> {
   ...
   * @param invoice Factura
   */
  public async generateAndSavePDF(invoice: Invoice): Promise<string> {
